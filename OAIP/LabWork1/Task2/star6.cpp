...
void Star6 :: paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        points[i * 2] = QPoint( int(0 - Radius2 * sin(2 * M_PI / 6 * i)) , int(0 - Radius1 * cos(2 * M_PI / 6 * i)) );
        points[i * 2 + 1] = QPoint( int(0 - (double) Radius2 / 2 * sin(2 * M_PI / 6 * i + M_PI / 6)) , int(0 - (double) Radius1 / 2 * cos(2 * M_PI / 6 * i + M_PI / 6)) );
    }
